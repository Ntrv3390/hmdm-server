...
DECLARE
    ...
    v_ts BIGINT;
BEGIN
    -- Get the specific device by number h0001
    SELECT id, customerid INTO v_device_id, v_customer_id FROM devices WHERE number = 'h0001' LIMIT 1;
    
    IF v_device_id IS NOT NULL THEN
        RAISE NOTICE 'Inserting demo GPS data for device h0001 (ID: %)', v_device_id;
    ...
    ELSE
        RAISE EXCEPTION 'Device h0001 not found! Please check the device number.';
    END IF;
END $$;
